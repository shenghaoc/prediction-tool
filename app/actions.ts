...
const ml_model_map_promise: Promise<any[]> =
	prisma.$queryRaw`SELECT ml_models.name, intercept_map, month_map, 
...
const schema = z.object({
	...
	lease_commence_date: z.coerce
		.date()
		.min(new Date('1960-01-01'), { message: 'Must not be before 1960' })
...
export const funPredict = async (values: Schema) => {
	...
	const full_map = (await ml_model_map_promise).find(
		(element) =>
			element.name == parsed.ml_model &&
				...
				data: labels.map(
					(x) =>
						Math.round(
							Math.max(
								0,
								full_map['intercept_map'] +
									month_map.find((element) => element.name == x).value * full_map['month_map'] +
									full_map['town_map'] +
									storey_range_map.find((element) => element.name == parsed.storey_range).value *
										full_map['storey_range_map'] +
									parsed.floor_area_sqm * full_map['floor_area_sqm_map'] +
									full_map['flat_model_map'] +
									dayjs(parsed.lease_commence_date).year() * full_map['lease_commence_date_map']
							) * 100
						) / 100
				),
