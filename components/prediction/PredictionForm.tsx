...
function BtnPrimary({ children, loading }: { children: React.ReactNode; loading: boolean }) {
  ...
  return (
    <button
      ...
      onClick={handleClick}
      disabled={loading}
      className={`ripple-container rounded-btn bg-primary text-on-primary shadow-button flex min-h-[42px] cursor-pointer items-center justify-center border-none px-6 text-sm font-semibold transition hover:-translate-y-px active:translate-y-0 disabled:cursor-not-allowed disabled:opacity-50 disabled:pointer-events-none${loading ? " animate-pulse" : ""}`}
    >
      {children}
    </button>
  );
...
export default function PredictionForm({
  ...
  const selectClass =
    "field-select w-full cursor-pointer appearance-none rounded-input border border-border bg-input-bg pl-3.5 pr-9 text-sm font-medium text-text outline-none transition hover:border-border-strong focus:border-primary focus:ring-[3px] focus:ring-primary-ring min-h-[42px]";
      ...
      <form onSubmit={handleSubmit}>
        <div className="grid grid-cols-2 gap-x-4 gap-y-3.5 max-sm:grid-cols-1">
          ...
          <Field label={t("floor_area")} full htmlFor="input-floor_area">
            <div className="grid grid-cols-[1fr_auto]">
              <input
                ...
                inputMode="numeric"
                enterKeyHint="done"
                className="rounded-input rounded-r-none border border-border bg-input-bg px-3.5 text-sm font-medium text-text outline-none transition hover:border-border-strong focus:border-primary focus:ring-[3px] focus:ring-primary-ring min-h-[42px] w-full cursor-text placeholder:text-text-muted placeholder:font-normal"
          ...
          <div className="col-span-2 mt-1 grid grid-cols-2 gap-3 max-sm:col-span-1 max-sm:grid-cols-1">
            ...
            <button type="button" className="rounded-btn flex min-h-[42px] cursor-pointer items-center justify-center border border-border bg-transparent px-6 text-sm font-semibold text-text-secondary transition hover:-translate-y-px active:translate-y-0" onClick={onReset}>
              {t("reset_form")}
            </button>
          </div>
        </div>
